...
const Register = () => {
  ...
  const validate = () => {
    ...
    if (!/^[A-Za-z]+$/.test(form.username)) return 'Username must contain only letters';
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      const { data } = await authAPI.register({   // Register new user
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        {/* Logo — flexbox centers the icon inside the circle */}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Submit button — flexbox centers icon and text */}
